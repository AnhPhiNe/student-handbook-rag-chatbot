...
- `$ARGUMENTS` may contain:
  ...
  - `--auto-update` â€” Enable automatic graph updates on commit (writes `autoUpdate: true` to `$UA_DIR/config.json`)
  - `--no-auto-update` â€” Disable automatic graph updates (writes `autoUpdate: false` to `$UA_DIR/config.json`)
  - `--review` â€” Run full LLM graph-reviewer instead of inline deterministic validation
  - `--language <lang>` â€” Generate all textual content (summaries, descriptions, tags, titles, languageNotes, languageLesson) in the specified language. Accepts ISO 639-1 codes (`zh`, `ja`, `ko`, `en`, `es`, `fr`, `de`, etc.) or friendly names (`chinese`, `japanese`, `korean`, `english`, `spanish`, etc.). Locale variants supported: `zh-TW`, `zh-HK`, etc. Defaults to `en` (English). Stores preference in `$UA_DIR/config.json` for consistency across incremental updates.
  - `--exclude <patterns>` â€” Comma-separated glob patterns for additional files/directories to exclude from analysis (e.g., `--exclude "tests/*,docs/*"`). These patterns take highest priority over built-in defaults and `.understandignore` rules. Supports gitignore syntax including `!` negation.
  - A directory path (e.g. `/path/to/repo` or `../other-project`) â€” Analyze the given directory instead of the current working directory
...
1. **Resolve `PROJECT_ROOT`:**
   ...
   - **Worktree redirect.** If `PROJECT_ROOT` is inside a git worktree (not the main checkout), redirect output to the main repository root. Worktrees managed by Claude Code are ephemeral â€” the data directory (`.ua/`, or legacy `.understand-anything/`) written there is destroyed when the session ends, taking the knowledge graph with it (issue #133). Detect a worktree by comparing `git rev-parse --git-dir` against `git rev-parse --git-common-dir`; in a normal checkout or submodule they resolve to the same path, in a worktree they differ and the parent of `--git-common-dir` is the main repo root.
     ...
     Set `UNDERSTAND_NO_WORKTREE_REDIRECT=1` if you intentionally want a per-worktree graph (rare â€” most users want the redirect).
1.5. **Ensure the plugin is built.** Later phases invoke Node scripts that import `@understand-anything/core`. On a fresh install `packages/core/dist/` does not exist yet â€” build once.
   ...
   **Important:** do **not** assume the plugin root is simply two directories above the skill path string. In many installations `~/.agents/skills/understand` is a symlink into the real plugin checkout. Prefer runtime-provided plugin roots first (for Claude), then fall back to universal symlinks, skill symlink resolution, and common clone-based install paths.
   ...
   If `pnpm` is missing, report to the user: "Install Node.js â‰¥ 22 and pnpm â‰¥ 10, then re-run `/understand`."
...
3.5. **Auto-update configuration:**
 ...
 3.6. **Language configuration:**
    ...
    - **Language code normalization:** Map friendly names to ISO codes:
      - `chinese` â†’ `zh`, `japanese` â†’ `ja`, `korean` â†’ `ko`, `english` â†’ `en`, `spanish` â†’ `es`, `french` â†’ `fr`, `german` â†’ `de`, `portuguese` â†’ `pt`, `russian` â†’ `ru`, `arabic` â†’ `ar`, etc.
    ...
    - If `--language` is NOT specified:
      ...
      - **Otherwise detect (first run only).** Infer the predominant language of the user's conversation as an ISO 639-1 code (`$DETECTED_LANG`). If it is `en` or cannot be confidently determined, set `$OUTPUT_LANGUAGE=en` and proceed silently â€” no prompt (English users see no change).
 ...
 3.7. **Exclude patterns:**
    ...
    - These patterns take highest priority â€” they are applied on top of default patterns and `.understandignore` rules. Use `!` prefix to force-include files that would otherwise be excluded.
...
## Phase 0.5 â€” Ignore Configuration
...
If the script exits non-zero, the failure is hard â€” relay the full stderr to the user as a Phase 1.5 failure. Do not attempt to recover; the script's internal fallback (count-based) already handles recoverable issues. A non-zero exit means a fundamental problem (missing input file, malformed JSON, etc.).
...
Report: `[Phase 2/7] Analyzing files â€” <totalFiles> files in <totalBatches> batches (up to 5 concurrent)...`
...
>
> Pre-resolved import data for this batch (use directly â€” do NOT re-resolve imports from source):
> ```json
...
> ```
>
> Files to analyze in this batch (every entry MUST be passed through to `batchFiles` with all four fields â€” `path`, `language`, `sizeLines`, `fileCategory`):
...
The merge script also runs a `tested_by` linker that canonicalizes test-coverage edges in two passes. **Pass 1** walks LLM-emitted `tested_by` edges and flips inverted ones in place; semantically broken edges (testâ†”test, prodâ†”prod, orphan endpoints) are dropped. **Pass 2** supplements with path-convention pairings. Production nodes that end up sourcing any `tested_by` edge get a `"tested"` tag. All resulting edges run `production â†’ test`.
...
2. **Generate structural fingerprints baseline.** This creates the basis for future automatic incremental updates and **must succeed before `meta.json` is written** â€” otherwise auto-update sees a fresh commit hash with no fingerprints to compare against, classifies every file as STRUCTURAL, and escalates to `FULL_UPDATE` on every subsequent commit (issue #152).
...
4. Clean up intermediate files, **preserving `scan-result.json`** so future incremental runs can skip Phase 1 SCAN (see issue #293). We `mv` scratch dirs into a timestamped `.trash-*` instead of `rm -rf`ing them directly â€” this avoids tripping destructive-action gates on hardened hosts (e.g. freshness-window checks) that flag deleting directories created moments earlier (see issue #301). The delayed-purge step in Phase 0 reclaims the space once the trash is older than 7 days.
   ```bash
   # Preserve scan-result.json â€” Phase 1's deterministic file inventory.
   # Future incremental runs (Phase 2 compute-batches.mjs --changed-files=â€¦)
