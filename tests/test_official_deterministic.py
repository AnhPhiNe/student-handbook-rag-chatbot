...
from src.evaluation.dataset import _validate_deterministic_v9_contract
from src.evaluation.suites import _evaluate_v7_outcome_case, _v8_task_execution_checks
...
def test_all_contracts_accept_their_fixture_and_reject_wrong_plan_cohort():
    for case in build():
        ...
        row = _evaluate_v7_outcome_case(case, result, started=time.perf_counter())
        ...
        if case["accepted_outcomes"][0]["required_tasks"]:
            ...
            assert not _evaluate_v7_outcome_case(case, bad, started=time.perf_counter())["passed"]
...
def test_fact_lock_cannot_pass_using_full_display_table():
    ...
    assert _v8_task_execution_checks(expected, [task])["resolved_result"] is False


def test_list_slot_is_compared_without_hashing_or_automatic_acceptance():
    from src.evaluation.suites import _v7_task_matches
    actual = {"slots": {"requested_field": ["email", "office"]}}
    assert not _v7_task_matches({"slot_value_alternatives": {"requested_field": ["all"]}}, actual)
    assert _v7_task_matches({"slot_value_alternatives": {"requested_field": [["office", "email"]]}}, actual)
...
def test_evaluator_exception_retains_raw_runtime_result(monkeypatch):
    ...
    monkeypatch.setattr(suites, "_evaluate_v7_outcome_case", fail)
...
def test_contact_field_list_is_accepted_only_with_both_outputs():
    ...
    assert not _evaluate_v7_outcome_case(case, result, started=time.perf_counter())["passed"]
...
def test_full_table_assertion_rejects_missing_rows():
    ...
    checks = _v8_task_execution_checks(expected, [task])
...
def test_execution_evidence_cannot_borrow_explicit_wrong_cohort():
    ...
    assert _v8_task_execution_checks(expected, [task])["evidence_fields"] is False
...
def test_narrow_question_requires_requested_fact_not_whole_record():
    ...
    assert _evaluate_v7_outcome_case(cases[48], result, started=time.perf_counter())["passed"]
    result["task_results"][0]["evidence"][0]["resolved_result"]["result"]["multiplier"] = 1.5
    assert not _evaluate_v7_outcome_case(cases[48], result, started=time.perf_counter())["passed"]
...
def test_compound_contract_rejects_dropped_task_and_accepts_reordering():
    for case in build():
        ...
        assert not _evaluate_v7_outcome_case(case, result, started=time.perf_counter())["passed"], case["id"]
