    ...
    def rerank(
        ...
        try:
            ...
            reranked_results = sorted(
                reranked_results, key=lambda x: x["rerank"]["final_score"], reverse=True
            )
            return reranked_results[:top_n]

        except Exception as e:
            print(
                f"[Reranker] Error during reranking: {e}. Fallback to default ranking."
            )
            ...
            reranked = sorted(
                results, key=lambda x: x["rerank"]["final_score"], reverse=True
            )
