...
- **Graceful Quality Degradation**: If the primary LLM (`llama-3.3-70b-versatile`) exhausts all keys, the system sequentially degrades to `gpt-oss-120b`, `qwen3.6-27b`, and finally `llama-3.1-8b-instant`.

### 2. 🚀 Two-Tier Caching System
- **L1 Cache (Redis via Upstash):** A centralized in-memory caching layer that allows multi-instance backend deployments to share state.
- **L2 Cache (Local JSON Fallback):** Fallback JSON cache for zero-downtime.

### 3. ☁️ Multi-Cohort Parent-Child Storage Architecture
- **MongoDB Atlas (DocStore):** Centralized "Parent" storage holding full, uncut regulation documents for both K48 and K51 cohorts.
- **Qdrant Cloud (VectorDB):** Stores highly-focused, overlapping 200-token "Child" chunks that act as semantic pointers to the MongoDB documents.

### 4. 🧬 Dynamic Multi-Cohort Pipeline
- Fully autonomous ingestion pipeline capable of parsing multiple student handbooks (e.g., K48, K51) simultaneously with collision-free ID management.
