...
def get_max_token_limits() -> dict[str, int]:
    """Return token limits for each generated chunk type."""
    return {
        "regulation": 850,
        "table": 250,
        "formula": 180,
...
def detect_overlong_chunks(
    ...
    """Report chunks that exceed the limit configured for their type."""
    ...
    for chunk in chunks:
        ...
        if limit is None:
            # Ignore chunk types without a configured token limit.
            continue
...
def build_chunk_report(
    ...
    """Summarize chunk counts, token sizes, and validation findings."""
    ...
    return {
        ...
        "avg_token_count_approx": round(
            ...
            / max(len(all_chunks), 1),  # Avoid division by zero for an empty build.
            2,
        ),
        "max_token_count_approx": max(
            [chunk["token_count_approx"] for chunk in all_chunks],
            default=0,  # Default to zero for an empty build.
        ),
