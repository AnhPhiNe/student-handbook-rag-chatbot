...
def is_out_of_domain_query(query: str, retrieval_result: dict[str, Any]) -> bool:
    """
    Detect queries that are not actually answerable from the student handbook.

    This is not exact-string blocking. It checks whether:
    - the query has any student-handbook domain signal
    - the retrieved source type matches the query intent/domain
    - retrieval confidence is weak or irrelevant
    """
    if can_answer_deterministically(retrieval_result):
        return False
    ...
    has_domain_signal = _has_domain_signal(ascii_query)
    ...
    top_item = items[0]
    top_chunk_type = str(top_item.get("metadata", {}).get("chunk_type") or "")

    # Nếu query không hỏi đơn vị/liên hệ/khoa mà top source lại là directory,
    # khả năng cao retrieval đang kéo nguồn linh tinh.
    asks_directory_info = _contains_any(
        ascii_query,
        [
            "phong",
            "khoa",
    ...
    if (
        not asks_directory_info
        and top_chunk_type in {"office_directory", "faculty_program_directory"}
    ):
        return True
...
def build_clarification_question(query: str, retrieval_result: dict[str, Any]) -> str:
    ...
    if ambiguity_kind == "information_technology":
        return (
            "Bạn muốn hỏi Phòng Công nghệ Thông tin hay "
