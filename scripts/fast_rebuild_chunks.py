    ...
    for cohort, config_path in cohorts:
        ...
        for file in chunk_dir.glob("*.json"):
            if file.name.startswith(f"{cohort}_"):
                continue
            if file.name in ["all_chunks.json", "semantic_chunks.json", "regulation_chunks.json", "directory_chunks.json", "all_docstore_items.json", "formula_chunks.json", "table_chunks.json"]:
